...
# Usage: REPO=org/thunderid BRANCH=main REQUIRED_CHECKS="api-lint,breaking-changes,contract-tests,coverage-meta,exemptions" ./check-branch-protection.sh

set -euo pipefail

REPO="${REPO:?set REPO=org/repo}"
BRANCH="${BRANCH:-main}"
REQUIRED_CHECKS="${REQUIRED_CHECKS:-api-lint,breaking-changes,contract-tests,coverage-meta,exemptions}"
