...
// The budget for those is zero, so a test that claims an operation without
// exercising it fails the check.
//
...
// Operations whose only test is a t.Skip stub. Zero: a skipped test asserts nothing,
// so it is treated as no coverage at all. Raising this re-admits tests that claim an
// operation without exercising it, so it should be a deliberate, reviewed decision.
const MAX_SCAFFOLDED = Number(process.env.MAX_SCAFFOLDED ?? 0);
