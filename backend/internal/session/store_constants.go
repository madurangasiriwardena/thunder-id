...
var (
	...
	//       $15=SESSION_STATE, $16=AUTH_FACTORS, $17=VERSION
	queryCreateSession = dbmodel.DBQuery{
		...
		Query: `INSERT INTO "SESSION_RECORD" ` +
			...
			`HANDLE_EXPIRES_AT, BINDING_TYPE, SESSION_STATE, AUTH_FACTORS, VERSION) ` +
	...
	queryGetSessionByHandle = dbmodel.DBQuery{
		...
		Query: `SELECT SESSION_ID, SUBJECT_ID, SESSION_GROUP_ID, AUTHENTICATED_AT, ` +
			`ASSURANCE_LEVEL, AUTH_FACTORS, CREATED_AT, LAST_ACTIVE_AT, IDLE_EXPIRES_AT, ` +
	...
	// queryGetActiveSessionBySubjectAndGroup retrieves the ACTIVE SESSION_RECORD for a
	// (SUBJECT_ID, SESSION_GROUP_ID) pair. Used by CreateSessionFromFlow for find-or-create.
	// Args: $1=SUBJECT_ID, $2=SESSION_GROUP_ID, $3=DEPLOYMENT_ID
	queryGetActiveSessionBySubjectAndGroup = dbmodel.DBQuery{
		ID: "SSQ-SRS-05",
		Query: `SELECT SESSION_ID, SUBJECT_ID, SESSION_GROUP_ID, AUTHENTICATED_AT, ` +
			`ASSURANCE_LEVEL, AUTH_FACTORS, CREATED_AT, LAST_ACTIVE_AT, IDLE_EXPIRES_AT, ` +
			`ABSOLUTE_EXPIRES_AT, HANDLE_ID, HANDLE_ISSUED_AT, HANDLE_EXPIRES_AT, ` +
			`BINDING_TYPE, SESSION_STATE, VERSION ` +
			`FROM "SESSION_RECORD" ` +
			`WHERE SUBJECT_ID = $1 AND SESSION_GROUP_ID = $2 AND SESSION_STATE = 'ACTIVE' ` +
			`AND DEPLOYMENT_ID = $3 LIMIT 1`,
	}

	// queryUpdateSessionAuth updates AUTH_FACTORS, ASSURANCE_LEVEL, and AUTHENTICATED_AT for
	// an existing ACTIVE session during SSO factor augmentation.
	// Args: $1=SESSION_ID, $2=AUTH_FACTORS, $3=ASSURANCE_LEVEL, $4=AUTHENTICATED_AT, $5=DEPLOYMENT_ID
	queryUpdateSessionAuth = dbmodel.DBQuery{
		ID: "SSQ-SRS-06",
		Query: `UPDATE "SESSION_RECORD" ` +
			`SET AUTH_FACTORS = $2, ASSURANCE_LEVEL = $3, AUTHENTICATED_AT = $4 ` +
			`WHERE SESSION_ID = $1 AND DEPLOYMENT_ID = $5`,
	}
)
