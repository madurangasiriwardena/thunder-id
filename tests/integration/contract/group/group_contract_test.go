...
// it. coverage-meta resolves each name against the test functions in this package: the
// function must exist, and one whose body only calls t.Skip counts as scaffolded rather
// than covered. Naming a test here therefore cannot certify coverage on its own.
var coveredOperations = map[string]string{
	...
	"listGroupMembers":   "TestGroupMembersScaffold",
	"addGroupMembers":    "TestGroupMembersScaffold",
	"removeGroupMembers": "TestGroupMembersScaffold",
}
